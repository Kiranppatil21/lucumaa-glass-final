    ...
    try:
        ...
        if data.cutouts:
            ...
            for cutout in cutouts_to_show:
                size = f"Ø{cutout.diameter:.2f}" if cutout.diameter else f"{cutout.width:.2f}×{cutout.height:.2f}"
                edges = f"{cutout.left_edge:.2f}/{cutout.right_edge:.2f}/{cutout.top_edge:.2f}/{cutout.bottom_edge:.2f}"
                cutout_rows.append([
                    cutout.number,
                    cutout.type,
                    size,
...
async def export_pdf_multipage(data: PDFExportRequest, user=Depends(get_erp_user)):
    ...
    try:
        ...
        # Page 1: Title and 2D Drawing (compact layout)
        title_style = ParagraphStyle('Title', parent=styles['Heading1'], fontSize=14, alignment=1, spaceAfter=3*mm)
        elements.append(Paragraph("Glass Specification Sheet", title_style))
        elements.append(Paragraph(f"<para align='center'><font size='8' color='gray'>Generated: {datetime.now().strftime('%d %b %Y, %H:%M')}</font></para>", styles['Normal']))
        elements.append(Spacer(1, 3*mm))
        
        # Glass info table (compact)
        glass_data = [
        ...
        # 2D Drawing (smaller to fit more)
        drawing_width = 180*mm
        drawing_height = 100*mm
        ...
        for cutout in data.cutouts:
            ...
            if cutout.diameter:
                ...
                if cutout.type == 'Star':
                    ...
                    p.closePath()
                    drawing.add(p)
                elif cutout.type in ['Pentagon', 'Hexagon', 'Octagon']:
                    from reportlab.graphics.shapes import Polygon
                    sides = 5 if cutout.type == 'Pentagon' else (6 if cutout.type == 'Hexagon' else 8)
                    points = []
        ...
        # Cutout table with smart pagination (try to fit more on first page)
        if data.cutouts:
            elements.append(Spacer(1, 3*mm))
            elements.append(Paragraph("<b>Cutout Specifications</b>", ParagraphStyle('Heading3', parent=styles['Heading3'], fontSize=10, spaceAfter=2*mm)))
            
            cutout_headers = ["#", "Type", "Size", "Pos(X,Y)", "L", "R", "T", "B"]
            # More cutouts per page with compact layout
            cutouts_per_page_first = 20  # First page can fit more
            cutouts_per_page_next = 25   # Subsequent pages
            
            # First page cutouts
            first_page_count = min(cutouts_per_page_first, len(data.cutouts))
            remaining_cutouts = data.cutouts[first_page_count:]
            
            # Build first page table
            cutout_rows = [cutout_headers]
            for cutout in data.cutouts[:first_page_count]:
                size = f"\u00d8{cutout.diameter:.1f}" if cutout.diameter else f"{cutout.width:.1f}\u00d7{cutout.height:.1f}"
                cutout_rows.append([
                    cutout.number,
                    cutout.type,
                    size,
                    f"({cutout.x:.2f},{cutout.y:.2f})",
                    f"{cutout.left_edge:.1f}",
                    f"{cutout.right_edge:.1f}",
                    f"{cutout.top_edge:.1f}",
                    f"{cutout.bottom_edge:.1f}"
                ])
            
            cutout_table = Table(cutout_rows, colWidths=[12*mm, 20*mm, 22*mm, 30*mm, 15*mm, 15*mm, 15*mm, 15*mm])
            cutout_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1E293B')),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ...
            if remaining_cutouts:
                total_additional_pages = (len(remaining_cutouts) + cutouts_per_page_next - 1) // cutouts_per_page_next
                for page_num in range(total_additional_pages):
                    # Page break
                    elements.append(PageBreak())
                    
                    # Page header for additional pages
                    elements.append(Paragraph(f"Cutout Details (Continued - Page {page_num + 2})", title_style))
                    elements.append(Spacer(1, 3*mm))
                    
                    # Build rows for this page
                    start_idx = page_num * cutouts_per_page_next
                    end_idx = min(start_idx + cutouts_per_page_next, len(remaining_cutouts))
                    page_cutouts = remaining_cutouts[start_idx:end_idx]
                    
                    page_cutout_rows = [["#", "Type", "Size", "Position (x,y)", "Left Edge", "Right Edge", "Top Edge", "Bottom Edge"]]
                    for cutout in page_cutouts:
                        if cutout.type == "Circle":
                            size = f"⌀{cutout.diameter:.2f}mm"
                        else:
                            size = f"{cutout.width:.2f}×{cutout.height:.2f}mm"
                        
                        page_cutout_rows.append([
                            cutout.number,
                            cutout.type,
                            size,
                            f"({cutout.x:.2f}, {cutout.y:.2f})",
                            f"{cutout.left_edge:.2f}",
                            f"{cutout.right_edge:.2f}",
                            f"{cutout.top_edge:.2f}",
                            f"{cutout.bottom_edge:.2f}"
                        ])
                    
                    page_cutout_table = Table(page_cutout_rows, colWidths=[12*mm, 22*mm, 22*mm, 28*mm, 16*mm, 16*mm, 16*mm, 16*mm])
                    page_cutout_table.setStyle(TableStyle([
                        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1E293B')),
                        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
                        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
                    ...
                    elements.append(page_cutout_table)
