...
const GlassConfigurator3D = () => {
  ...
  const createOrUpdateCutoutMesh = (cutout, scale, scene) => {
    ...
    // Create new mesh using manufacturing-grade normalized coordinates
    let mesh;
    ...
    if (cutout.type === 'HR' || cutout.type === 'Heart') {
      ...
      // Corner Notch using normalized approach
  ...
  const createCutoutBorder = (cutout, scale, glassWidth, glassHeight, scene) => {
    ...
    // High-accuracy outlines so shapes never look circular
    if (cutout.type === 'HR') {
      ...
      // Circles and cylinders keep circular outline; rectangles use box
